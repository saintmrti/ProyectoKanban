...
module.exports.getRequirements = async (conn, date) => {
  ...
  const { data: pedido } = await conn.query(`
    SELECT p.id as idOrden, p.pedido, p.fecha, p.ajuste_carga, p.destino, c.id as idProducto, c.producto FROM Qualtia_Planeacion_ordenes AS p
...
module.exports.insertRequirement = async (conn, { products, date }) => {
  ...
  await conn.query(`
    INSERT INTO Qualtia_Planeacion_ordenes (fecha, idProducto, ajuste_carga, pedido, destino) VALUES
    ${products
      .map(
        (product) =>
          `('${dateTime}', ${product.idProducto}, ${product.ajuste_carga}, ${product.pedido}, '${product.destino}')`
      )
      .join(",")} 
  `);
